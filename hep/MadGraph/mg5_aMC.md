# Documentation for the `mg5_aMC` code


## Input

Flags:
- `--logging`: logging level (DEBUG|INFO|WARNING|ERROR|CRITICAL) [%default]
- `--file`: Use script file FILE
- `--mgme_dir`: Use MG_ME directory MGME_DIR
- `--web`: force to be in secure mode
- `--debug`: force to launch debug mode
- `--mode`: Define some additional command provide by a PLUGIN
...
Interaction between the user and mg5 happens through `cmd_line` (command line) class.
It is a `MasterCmd(Web)`class that deals with the interpretation and execution of commands.

Inside the `mg5_aMC` it is defined as:
...
The `MasterCmd` class is defined in `./madgraph/interface/master_interface.py`.

## Complete `mg5_aMC` source code
...
if options.plugin:
    ...
    if not plugin.new_interface:
        logging.warning("Plugin: %s do not define dedicated interface and should be used without the
--mode options" % options.plugin)
        sys.exit()
